...
# Callbacks for the first step classification
callbacks_cnn = [
    ModelCheckpoint('best_model_cnn1.h5', monitor='val_acc', save_best_only=True, verbose=1)
]

callbacks_vgg16 = [
    ModelCheckpoint('best_model_vgg161.h5', monitor='val_acc', save_best_only=True, verbose=1)
]

callbacks_resnet = [
    ModelCheckpoint('best_model_resnet1.h5', monitor='val_accuracy', save_best_only=True, verbose=1)
]

callbacks_densenet = [
    ModelCheckpoint('best_model_densenet1.h5', monitor='val_accuracy', save_best_only=True, verbose=1)
]
...
# Callbacks for the second step classification
callbacks_cnn2 = [
    ModelCheckpoint('best_model_cnn2.h5', monitor='val_acc', save_best_only=True, verbose=1)
]

callbacks_vgg162 = [
    ModelCheckpoint('best_model_vgg162.h5', monitor='val_acc', save_best_only=True, verbose=1)
]

callbacks_resnet2 = [
    ModelCheckpoint('best_model_resnet2.h5', monitor='val_accuracy', save_best_only=True, verbose=1)
]

callbacks_densenet2 = [
    ModelCheckpoint('best_model_densenet2.h5', monitor='val_accuracy', save_best_only=True, verbose=1)
]
...
for fold, (train_index, val_index) in enumerate(kf.split(x_train, y_train)):
    ...
    print('\n', f"Fold {fold + 1} - CNN Validation Accuracy: {max(history_cnn.history['val_acc'])}", '\n')
    acc_scores_cnn.append(max(history_cnn.history['val_acc']))
    ...
    print('\n', f"Fold {fold + 1} - VGG16 Validation Accuracy: {max(history_vgg16.history['val_acc'])}", '\n')
    acc_scores_vgg16.append(max(history_vgg16.history['val_acc']))
    ...
    print('\n', f"Fold {fold + 1} - ResNet Validation Accuracy: {max(history_resnet.history['val_accuracy'])}", '\n')
    acc_scores_resnet.append(max(history_resnet.history['val_accuracy']))
    ...
    print('\n', f"Fold {fold + 1} - DenseNet Validation Accuracy: {max(history_densenet.history['val_accuracy'])}", '\n')
    acc_scores_densenet.append(max(history_densenet.history['val_accuracy']))
    # acc_scores_densenet.append(score_densenet_cv[1])
   
# Load the best models for prediction
best_model_cnn = load_model('best_model_cnn1.h5')
best_model_vgg16 = load_model('best_model_vgg161.h5')
best_model_resnet = load_model('best_model_resnet1.h5')
best_model_densenet = load_model('best_model_densenet1.h5')

# Evaluate the predictions
score_cnn = best_model_cnn.evaluate(x_test, y_test)
score_vgg16 = best_model_vgg16.evaluate(x_test, y_test)
score_resnet = best_model_resnet.evaluate(x_test, y_test)
score_densenet = best_model_densenet.evaluate(x_test, y_test)
...
for fold2, (train_index2, val_index2) in enumerate(kf2.split(x_train_step2, y_train_step2)):
    ...
    print('\n', f"Fold {fold2 + 1} - CNN Validation Accuracy: {max(history_cnn2.history['val_acc'])}", '\n')
    acc_scores_cnn2.append(max(history_cnn2.history['val_acc']))
    ...
    history_vgg162 = model_vgg162.fit(
        ...
        batch_size=batch_size2,
        validation_data=(x_val_fold2, y_val_fold2),
        callbacks=callbacks_vgg162,
        verbose=2
    )

    # score_vgg16_cv2 = model_vgg162.evaluate(x_val_fold2, y_val_fold2)
    print('\n', f"Fold {fold2 + 1} - VGG16 Validation Accuracy: {max(history_vgg162.history['val_acc'])}", '\n')
    acc_scores_vgg162.append(max(history_vgg162.history['val_acc']))
    ...
    history_resnet2 = model_resnet2.fit(
        ...
        batch_size=batch_size2,
        validation_data=(x_val_fold2, y_val_fold2),
        callbacks=callbacks_resnet2,
        verbose=2
    )

    # score_resnet_cv2 = model_resnet2.evaluate(x_val_fold2, y_val_fold2)
    print('\n', f"Fold {fold2 + 1} - ResNet Validation Accuracy: {max(history_resnet2.history['val_accuracy'])}", '\n')
    acc_scores_resnet2.append(max(history_resnet2.history['val_accuracy']))
    ...
    history_densenet2 = model_densenet2.fit(
        ...
        batch_size=batch_size2,
        validation_data=(x_val_fold2, y_val_fold2),
        callbacks=callbacks_densenet2,
        verbose=2
    )

    # score_densenet_cv2 = model_densenet2.evaluate(x_val_fold2, y_val_fold2)
    print('\n', f"Fold {fold2 + 1} - DenseNet Validation Accuracy: {max(history_densenet2.history['val_accuracy'])}", '\n')
    acc_scores_densenet2.append(max(history_densenet2.history['val_accuracy']))
    # acc_scores_densenet2.append(score_densenet_cv2[1])
   
# Load the best models for prediction
best_model_cnn2 = load_model('best_model_cnn2.h5')
best_model_vgg162 = load_model('best_model_vgg162.h5')
best_model_resnet2 = load_model('best_model_resnet2.h5')
best_model_densenet2 = load_model('best_model_densenet2.h5')

# Evaluate the predictions
score_cnn2 = best_model_cnn2.evaluate(x_test_step2, y_test_step2)
score_vgg162 = best_model_vgg162.evaluate(x_test_step2, y_test_step2)
score_resnet2 = best_model_resnet2.evaluate(x_test_step2, y_test_step2)
score_densenet2 = best_model_densenet2.evaluate(x_test_step2, y_test_step2)
